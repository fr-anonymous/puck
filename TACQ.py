...
class TACQ(object):
    ...
    def copy(self):
        ...
        q.gp = self.gp.copy()
        q.filter = self.filter.copy()
        q.joins = self.joins.copy()
    ...
    def renameVariables(self, prefix='Q'):
        ...
        for t in self.gp:
            ...
            if t['timestamp'][0] == '?':
                ...
                if not new:
                    new = self.addVar(old=t['timestamp'])
                else:
                    new = new[0]
    ...
    def freeze(self):
        ...
        for t in self.gp:
            ...
            #if var[0] == '?':
            #    if var in self.constants.keys():
            #        tripplet['timestamp'] = self.constants[var]
            #    else:
            #        tripplet['ttmestamp'] = self.addConst(var)
            #else:
            #    tripplet['timestamp'] = var
...
#        WHERE { (?dp ns1:consumption ?consumption, ?ts) }
